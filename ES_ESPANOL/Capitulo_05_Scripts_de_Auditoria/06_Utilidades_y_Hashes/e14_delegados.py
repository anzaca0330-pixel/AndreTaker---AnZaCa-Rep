...
log_file = os.path.join(log_dir, "downloader_delegados.log")

# Clear/create the log file on startup
with open(log_file, "w", encoding="utf-8") as f:
    f.write("--- Delegados HTTP Downloader Log Started ---\n")
...
class E14DelegadosDownloader:
    def __init__(self, depto_filter=None, muni_filter=None, zona_filter=None, puesto_filter=None, output_dir=None):
        self.base_dir = os.path.dirname(os.path.abspath(__file__))
        if output_dir:
            self.output_dir = output_dir
        else:
            portable_drive = "/media/andrea-zabala-c/D A T A1/segundaVuelta/delegados_pdf"
            if os.path.exists("/media/andrea-zabala-c/D A T A1"):
                self.output_dir = portable_drive
            else:
                self.output_dir = os.path.join(self.base_dir, "segundaVuelta", "delegados_pdf")
        os.makedirs(self.output_dir, exist_ok=True)
        
        self.base_url = "https://e14segundavueltapresidente.registraduria.gov.co"
    ...
    def process_puesto(self, p):
        ...
        for mesa in mesas:
            if mesa.get("digitalizado") == 1 and mesa.get("nombre_archivo"):
                ...
                logger.info(f"Downloading Delegados E14: {depto_name} -> {muni_name} -> {mesa_suffix}")
                try:
    ...
    def process_department(self, depto_info):
        ...
        logger.warning(f"====== Thread Worker started for Delegados Department: {depto_name} ======")
        ...
        logger.warning(f"====== Thread Worker finished successfully for Delegados Department: {depto_name} ======")

    def run(self):
        logger.warning("Starting High-Speed Delegados API Downloader (Department Parallelization)...")
        self.load_index()
        ...
        # Priorizar departamentos con más anomalías estructurales (ej. 60, 88 Consulados)
        priority_list = ['60', '88']
        departments_to_run.sort(key=lambda x: (priority_list.index(x['code']) if x['code'] in priority_list else 999, x['code']))
        
        logger.warning(f"Discovered {total_depts} matching departments in Divipole. Prioritizing anomalies first.")
        ...
        logger.warning("====== Delegados Downloader Finished Successfully! ======")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Descargador Masivo de Formularios E14 de Delegados vía API HTTP.")
    ...
    downloader = E14DelegadosDownloader(
