...
def analyze_pdf_national(pdf_path):
    ...
    # 3. pdfimages (Detección real de múltiples capas por página)
    try:
        proc = subprocess.run(["pdfimages", "-list", pdf_path], capture_output=True, text=True, timeout=10)
        lines = [l for l in proc.stdout.splitlines() if l.strip()]
        if len(lines) > 2:
            data_lines = lines[2:] # Omitir las 2 líneas de cabecera
            pages_count = {}
            ...
            # Si alguna página individual tiene más de 1 imagen, ES multicapa (inyección)
