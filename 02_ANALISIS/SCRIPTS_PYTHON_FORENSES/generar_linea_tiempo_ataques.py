...
def generate_attacks_timeline_report():
    ...
    md_content = """# LÍNEA DE TIEMPO OFICIAL DE CIBERATAQUES, DENUNCIAS POLICIALES Y DERECHOS HUMANOS

**Investigadora / Beneficiaria Principal:** Andrea Zabala Carcamo (C.C. 43.925.102)  
...
timeline
    ...
    8 Junio 2026   : Ciberataque Masivo Post-Denuncia en Fiscalía
                   : Infección por Rootkit, BIOS Lenovo bloqueado (Ticket Soporte Lenovo), vector FIXD.
    10 Junio 2026  : T-Mobile, Persistencia Rootkit y Pérdida de Cuenta de Rescate
                   : Adquisición de nuevo equipo; ineficacia de Factory Wipe; secuestro de cuenta de rescate.
    13 Junio 2026  : Sabotaje en Vehículo (11:01 PM) y Llamada al 911
                   : Alteración eléctrica del auto; llamada al 911 (registro T-Mobile 3 min / audio Sheriff 2.01 min).
    ...
    Julio 2026     : Respuesta FBI (Devolución a Policía Local sin Herramientas) y Testimonio Univ. Phoenix
                   : FBI remite el caso al Sheriff local (sin capacidad técnica); testimonios en Univ. de Phoenix.
```
...
### 2.1. El Apoyo de los "Testigos Digitales" y la Protección en México
- **Verificación Independiente:** La protección y resguardo actual de la investigadora no surgió al azar, sino como respuesta directa de una **red internacional de veedores y "testigos digitales"** que evaluaron la solidez técnica de sus dictámenes E-14, verificaron las anomalías y decidieron brindarle apoyo y acompañamiento.
...
- **Seguimiento Académico (Universidad de Phoenix):** La investigadora ha rendido múltiples declaraciones y testimonios narrando la secuencia cronológica de los hechos ante la **Universidad de Phoenix (`login.phoenix.edu`)**.

---

### 2.2. Evidencias Técnicas Clave Preservadas en la Cadena de Custodia

1. **Ticket / Key de Servicio al Cliente LENOVO (Bloqueo de BIOS):**  
   *Registro oficial de soporte técnico emitido por Lenovo al reportar la inoperatividad y el bloqueo a nivel de hardware/BIOS del portátil ThinkPad derivado del ataque de Rootkit/Bootkit persistente.*
...
3. **Grabación de Audio .VMA del Departamento del Sheriff:**  
   *Archivo de audio `.vma` de 2.01 minutos remitido por la Buckingham County Sheriff's Office tras confrontarlos con el registro certificado de T-Mobile.*
4. **Dispositivo OBD-II (FIXD):**  
   *Dispositivo físico de diagnóstico automotriz preservado y sellado como vector de ataque físico/inalámbrico.*
...
| **Soporte Lenovo** | **Ticket / Key de Servicio Lenovo** | **Junio 2026** | **Certificado oficial de bloqueo BIOS por Rootkit** |
...
"""
    ...
    md_file = os.path.join(out_dir, "LINEA_DE_TIEMPO_INCIDENTES_Y_ATAQUES.md")
    txt_file = os.path.join(out_dir, "LINEA_DE_TIEMPO_INCIDENTES_Y_ATAQUES.txt")

    with open(md_file, "w", encoding="utf-8") as f:
        f.write(md_content)

    with open(txt_file, "w", encoding="utf-8") as f:
        f.write(md_content.replace("#", "").replace("```mermaid", "").replace("```", ""))

    os.system(f"cp -rv '{out_dir}'/LINEA_DE_TIEMPO_INCIDENTES_Y_ATAQUES.* '{drive_dir}'/")
    print("✅ Línea de tiempo actualizada con los hechos de la última semana de junio (T-Mobile 2da visita, Samsung S23, aislamiento Faraday y IC3/Sheriff).")

if __name__ == "__main__":
    generate_attacks_timeline_report()
